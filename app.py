...
def init_titan_states():
    ...
    if 'is_premium_active' not in st.session_state: st.session_state['is_premium_active'] = True
...
def get_recovery_protocol(mode: str, iron_target: str, current_time: datetime) -> str:
    ...
    if mode == "ClassOnly":
        return "".join([
            "<div class='titan-card'><h3 style='margin-top:0;'>🏥 العيادة الطبية (بروتوكول ما بعد الكارديو)</h3>",
            "<p style='color:#8B949E; text-align:right;'>بما أن مسارك اليوم هو <b>(كلاس لياقة فقط)</b>، الاستشفاء الحراري ممنوع طبياً.</p>",
            "<div class='med-neutral'><h4 style='color:#2EA043; margin:0;'>🏊 التبريد الهادئ وإعادة السوائل (Active Cool-down)</h4>",
            "<ul style='font-size:14px; margin-top:10px; color:#E8ECEF;'><li><b>السباحة الحرة:</b> 10 دقائق حركة بطيئة.</li><li><b>شرب الماء:</b> لتر كامل تدريجياً.</li></ul></div>",
            "<div class='med-danger'><h4 style='color:#D29922; margin:0;'>🚫 حظر حراري تام (No Heat Protocol)</h4>",
            "<p style='margin-top:5px; font-size:14px; color:#E8ECEF;'>يُمنع الدخول للساونا أو غرفة البخار اليوم لتجنب الهدم العضلي.</p></div></div>"
        ])
    elif is_heavy_session and mode in ["Full", "IronOnly"]:
        return "".join([
            "<div class='titan-card'><h3 style='margin-top:0;'>🏥 العيادة الطبية (العلاج التبايني العنيف)</h3>",
            "<p style='color:#8B949E; text-align:right;'>مسارك اليوم <b>عنيف (تمارين مقاومة ثقيلة)</b>. يجب التخلص من حمض اللاكتيك.</p>",
            "<div class='med-hot'><h4 style='color:#F85149; margin:0;'>🔥 المرحلة 1: التوسيع الوعائي</h4>",
            "<ul style='font-size:14px; margin-top:10px; color:#E8ECEF;'><li><b>غرفة البخار:</b> من 5 إلى 8 دقائق كحد أقصى.</li></ul></div>",
            "<div class='med-cold'><h4 style='color:#58A6FF; margin:0;'>🧊 المرحلة 2: الانقباض الوعائي</h4>",
            "<ul style='font-size:14px; margin-top:10px; color:#E8ECEF;'><li><b>الجاكوزي البارد:</b> 1-2 دقيقة مباشرة بعد الخروج من البخار.</li></ul></div>",
            "<div class='med-danger'><h4 style='color:#D29922; margin:0;'>⚠️ تحذير طبي للخصوبة</h4>",
            "<p style='margin-top:5px; font-size:14px; color:#E8ECEF;'>الختام إلزامي بالماء البارد جداً لحماية هرمون التستوستيرون والجهاز العصبي.</p></div></div>"
        ])
    else:
        return "".join([
            "<div class='titan-card'><h3 style='margin-top:0;'>🏥 العيادة الطبية (التبريد العميق Active Recovery)</h3>",
            "<p style='color:#8B949E; text-align:right;'>مسارك اليوم متوسط الشدة. ركز على الاستشفاء البارد النشط.</p>",
            "<div class='med-cold'><h4 style='color:#58A6FF; margin:0;'>🧊 التبريد وتقليل الالتهاب</h4>",
            "<ul style='font-size:14px; margin-top:10px; color:#E8ECEF;'><li><b>الجاكوزي البارد:</b> 3 إلى 5 دقائق מתواصلة.</li>",
            "<li><b>السباحة الحرة:</b> 15 دقيقة لتفكيك مفاصل العمود الفقري.</li></ul></div></div>"
        ])
...
# =====================================================================
# 9. COMMERCIAL NUTRITION DATABASE
# =====================================================================
    ...
    if st.session_state.get('last_sync_timestamp') != current_date:
        ...
        if not plan_df.empty and 'Date' in plan_df.columns and 'Status' in plan_df.columns:
            ...
            if not today_rows.empty:
                ...
                status_map = {"حضور كامل": "Full", "حديد فقط": "IronOnly", "كلاس فقط": "ClassOnly", "تأخير": "Delayed", "غائب": "Absent", "مجدول": "Full"}
...
def main():
    ...
    days_ar_map = {"Sunday": "الأحد", "Monday": "الاثنين", "Tuesday": "الثلاثاء", "Wednesday": "الأربعاء", "Thursday": "الخميس", "Friday": "الجمعة", "Saturday": "السبت"}
    ...
    st.markdown("".join([
        ...
        f"<div style='color: #8B949E; font-size: 16px; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px;'>مكة المكرمة | {today_ar_name} {current_date_str} | <span style='color: #E8ECEF;'>{CURRENT_MAKKAH_TIME.strftime('%I:%M %p')}</span></div>",
        f"<div style='display: flex; gap: 25px; align-items: center;'><span style='background: rgba(229, 185, 76, 0.15); padding: 10px 25px; border-radius: 35px; color: #E5B94C; font-weight: 900; font-size: 14px; border: 1px solid rgba(229, 185, 76, 0.3);'>👑 ENTERPRISE PRO ACTIVE</span>",
        f"<span style='color: #E8ECEF; font-weight: 800; font-size: 20px; letter-spacing: 1px;'>Titan System V60</span></div></div>"
    ]), unsafe_allow_html=True)

    t_ops, t_setup, t_log, t_clinic, t_vision, t_fuel, t_sys = st.tabs(["🚀 الميدان والملاحة", "🗓️ المخطط الأسبوعي", "🏋️ السجل الحيوي", "🏥 العيادة الطبية", "📸 عدسة الذكاء (AI)", "🥗 مختبر الماكروز", "🛠️ الإدارة والإصلاح"])
    ...
    with t_ops:
        ...
        if not plan_df.empty and 'Date' in plan_df.columns:
            ...
            if not today_row.empty: todays_scheduled_class = str(today_row.iloc[0].get('Class', 'موتيف 8'))
        ...
        if today_ar_name == "الجمعة" and mode != "IronOnly":
            st.markdown("".join(["<div class='titan-card titan-card-center' style='border: 2px solid #2EA043; background: linear-gradient(145deg, rgba(46, 160, 67, 0.1), #080A0F);'><h1 style='color:#2EA043; margin:0; font-size: 55px;'>يوم راحة سلبي إلزامي 🛑</h1><p style='color:#8B949E; margin-top:20px; font-size:22px; font-weight: bold;'>الأنسجة العضلية تُبنى أثناء النوم والراحة، وليس في النادي. استمتع بيومك يا مهندس.</p></div>"]), unsafe_allow_html=True)
            if st.button("الذهاب للنادي للحديد فقط (تجاوز الراحة)", use_container_width=True): update_attendance_decision("IronOnly", "حديد فقط", current_date_str); st.rerun()
                
        elif todays_scheduled_class == "راحة / غياب" or mode == "Absent":
            st.markdown("".join([f"<div class='titan-card' style='border-color: #F85149; background: linear-gradient(145deg, rgba(248, 81, 73, 0.05), #080A0F);'><h2 style='color:#F85149; text-align:center; font-size: 40px;'>مجدول كـ (راحة / غياب) ❌</h2><p style='text-align:center; color:#8B949E; font-size:20px;'>تم ترحيل تمرين العضلة المستهدفة <b>({iron_target})</b> للغد تلقائياً.</p><hr style='border-color:#30363D; margin: 35px 0;'><h4 style='color:#E8ECEF; text-align:center;'>توجيه تغذية طارئ</h4><p style='text-align:center; color:#8B949E; font-size:16px;'>بما أن الحرق اليوم منخفض، يُنصح بتقليل حصة الكربوهيدرات والدهون وزيادة البروتين للحفاظ على الكتلة.</p></div>"]), unsafe_allow_html=True)
            if st.button("إلغاء الراحة والتوجه للنادي الآن", use_container_width=True): update_attendance_decision("Full", "حضور كامل", current_date_str); st.rerun()
            ...
            with col_ops_r:
                ...
                st.markdown("".join([f"<div style='background: rgba(88, 166, 255, 0.08); padding: 20px; border-radius: 15px; margin-top: 25px; border: 1px solid rgba(88, 166, 255, 0.2); text-align: right;'><p style='color:#8B949E; margin: 0 0 12px 0; font-size:16px;'><span style='color:#58A6FF;'>📏 المسافة:</span> <b style='color:#E8ECEF; font-size:19px;'>{dist_val:.1f} KM</b></p><p style='color:#8B949E; margin: 0; font-size:16px;'><span style='color:#58A6FF;'>⏱️ الوقت (Traffic):</span> <b style='color:#E8ECEF; font-size:19px;'>{eta_val} دقيقة</b></p></div>"]), unsafe_allow_html=True)
                ...
                if st.button("✅ حضور كامل (كلاس + حديد)", use_container_width=True): update_attendance_decision("Full", "حضور كامل", current_date_str); st.rerun()
                if st.button("🏋️ صالة الحديد فقط", use_container_width=True): update_attendance_decision("IronOnly", "حديد فقط", current_date_str); st.rerun()
                if st.button("🤸 كلاس اللياقة فقط", use_container_width=True): update_attendance_decision("ClassOnly", "كلاس فقط", current_date_str); st.rerun()
                if st.button("⏳ تأخير مسار (زحمة الطريق)", use_container_width=True): update_attendance_decision("Delayed", "تأخير", current_date_str); st.rerun()
                if st.button("❌ تسجيل غياب لليوم", use_container_width=True): update_attendance_decision("Absent", "غائب", current_date_str); st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)

            with col_ops_l:
                n_str, a_str, is_str, ie_str, arr_obj, d_km, e_min = get_dynamic_schedule(mode, origin_loc, CURRENT_MAKKAH_TIME)
                time_warning_html = "<div class='alert-box' style='margin-top:25px;'>⚠️ تنبيه هندسي للوقت: الكلاس يبدأ 9:00 م. وصولك الآن مبكر جداً، ستحتاج للعودة لاحقاً، أو يمكنك إنهاء تمرينك دفعة واحدة بتحويل المسار لـ (حديد فقط).</div>" if arr_obj.hour < 18 and mode in ["Full", "ClassOnly"] else ""
                
                if mode == "Full":
                    main_ops_html = "".join(["<div class='titan-card'><h3 style='margin-top:0;'>🗺️ الخطة أ (طاقة قصوى - كلاس وحديد)</h3>", f"<p><span class='data-label'>عضلة اليوم:</span> <b style='color:#E5B94C; font-size:22px;'>{iron_target}</b></p>", f"<p><span class='data-label'>كلاس السهرة:</span> <b style='color:#E5B94C; font-size:22px;'>{todays_scheduled_class}</b> <span style='font-size:14px; color:#F85149;'>(~{burn_est} kcal)</span></p>", f"<div style='background: rgba(229, 185, 76, 0.05); padding: 20px; border-radius: 12px; border-right: 5px solid #E5B94C; margin-top: 20px;'><p style='color:#E8ECEF; margin:0; font-size:16px;'><b>الاستراتيجية:</b> {flow_note}</p></div><hr style='border-color:#30363D; margin: 30px 0;'>", f"<p style='font-size: 17px;'>🚗 الانطلاق المجدول: <b style='color:#E8ECEF;'>{n_str}</b> | 🅿️ وصول البوابة: <b style='color:#E8ECEF;'>{a_str}</b></p><h5 style='margin-top:35px; color:#E5B94C;'>الجدول الزمني الميداني المقترح</h5>", f"<p style='font-size: 17px; margin: 12px 0;'>🔥 {a_str} - {is_str} : <span style='color:#8B949E;'>إحماء وتجهيز مفاصل دقيق</span></p>", f"<p style='font-size: 17px; margin: 12px 0;'>💪 {is_str} - {ie_str} : <b style='color:#F85149;'>صالة الحديد (تم تقييدها بـ 75 دقيقة لمنع الهدم)</b></p>", f"<p style='font-size: 17px; margin: 12px 0;'>🤸 09:00 PM - 09:50 PM : <b style='color:#2EA043;'>حضور الكلاس (مرحلة حرق الدهون الصافية)</b></p>", time_warning_html, "</div>"])
                elif mode == "IronOnly":
                    main_ops_html = "".join(["<div class='titan-card' style='border-color: #58A6FF; box-shadow: 0 15px 35px rgba(88, 166, 255, 0.1);'><h3 style='margin-top:0; color:#58A6FF;'>🏋️ مسار الحديد المكثف (تم إلغاء الكلاس)</h3>", f"<p><span class='data-label'>المستهدف الرئيسي:</span> <b style='color:#E5B94C; font-size:22px;'>{iron_target}</b></p>", "<div style='background: rgba(88, 166, 255, 0.05); padding: 20px; border-radius: 12px; border-right: 5px solid #58A6FF; margin-top: 20px;'><p style='color:#E8ECEF; margin:0; font-size:16px;'>كامل طاقتك موجهة لكسر الأوزان الحرة وبناء الكتلة العضلية اليوم.</p></div><hr style='border-color:#30363D; margin: 30px 0;'>", f"<p style='font-size: 17px;'>🚗 الانطلاق المجدول: <b style='color:#E8ECEF;'>{n_str}</b> | 🅿️ وصول البوابة: <b style='color:#E8ECEF;'>{a_str}</b></p><h5 style='margin-top:35px; color:#58A6FF;'>الجدول الميداني المفتوح</h5>", f"<p style='font-size: 17px; margin: 12px 0;'>🔥 {a_str} - {is_str} : <span style='color:#8B949E;'>إحماء دقيق وتمديد للأوتار</span></p>", f"<p style='font-size: 17px; margin: 12px 0;'>💪 {is_str} - {(arr_obj + timedelta(minutes=90)).strftime('%I:%M %p')} : <b style='color:#F85149;'>صالة الحديد المفتوحة (العب جولات إضافية للأجزاء الضعيفة)</b></p></div>"])
                elif mode == "ClassOnly":
                    main_ops_html = "".join(["<div class='titan-card' style='border-color: #E5B94C;'><h3 style='margin-top:0; color:#E5B94C;'>🤸 مسار الكارديو (صالة الحديد ملغية)</h3>", f"<p><span class='data-label'>الكلاس المجدول:</span> <b style='color:#E5B94C; font-size:22px;'>{todays_scheduled_class}</b></p>", "<div style='background: rgba(229, 185, 76, 0.05); padding: 20px; border-radius: 12px; border-right: 5px solid #E5B94C; margin-top: 20px;'><p style='color:#E8ECEF; margin:0; font-size:16px;'>الهدف اليوم هو حرق السعرات ورفع اللياقة القلبية. ركز على كفاءة التنفس.</p></div><hr style='border-color:#30363D; margin: 30px 0;'>", f"<p style='font-size: 17px;'>🤸 09:00 PM - 09:50 PM : <b style='color:#2EA043;'>حضور الكلاس (مستهدف حرق ~{burn_est} kcal)</b></p>", time_warning_html, "</div>"])
                else: 
                    main_ops_html = "".join(["<div class='titan-card' style='border-color: #F85149;'><h3 style='margin-top:0; color:#F85149;'>⚠️ مسار التأخير والطوارئ (إنقاذ التمرين)</h3>", f"<p><span class='data-label'>الحديد المختصر:</span> <b style='color:#E5B94C; font-size:22px;'>{iron_target}</b></p><hr style='border-color:#30363D; margin: 30px 0;'>", "<p style='font-size: 17px; margin: 12px 0;'>🤸 09:00 PM - 09:50 PM : <b style='color:#E5B94C;'>توجه للكلاس مباشرة لعدم تفويت التسخين الجماعي</b></p>", "<p style='font-size: 17px; margin: 12px 0;'>💪 09:55 PM - 10:30 PM : <b style='color:#F85149;'>حديد سريع ومكثف (استخدم أجهزة العزل فقط لحماية المفاصل المرهقة)</b></p></div>"])
    ...
    # -----------------------------------------------------------------
    # TAB 2: WEEKLY PLAN (هندسة الأسبوع السحابية) المأمن بـ get
    # -----------------------------------------------------------------
    with t_setup:
        ...
        with st.form("weekly_setup_master_form"):
            ...
            for index, (day_n, day_dt) in enumerate(week_dates_dict.items()):
                ...
                with cols_grid[index % 3]:
                    st.markdown(f"<div style='background: #161B22; padding: 20px; border-radius: 15px; margin-bottom: 20px; border: 1px solid #30363D;'><h5 style='color:#E8ECEF; text-align:right; margin: 0 0 15px 0;'>{day_n} <br><span style='font-size:12px; color:#8B949E;'>({day_dt})</span></h5>", unsafe_allow_html=True)
    ...
    with t_log:
        ...
        pre_workout_pain = st.selectbox("كيف تقيم حالة جسمك ومفاصلك اليوم قبل البدء بالأوزان؟", ["سليم 100% (جاهز لكسر الأوزان الحرة وبناء الكتلة)", "إرهاق عام وعضلات مشدودة (DOMS طبيعي من الأمس)", "ألم خفيف في أحد المفاصل (ركبة، كوع، رسغ)", "ألم حاد في أسفل الظهر أو الكتف الداخلي (خطر إصابة)"])
        if "المفاصل" in pre_workout_pain or "خطر" in pre_workout_pain: st.warning("🚨 تحذير النظام الطبي: يُمنع اليوم لعب الأوزان الحرة المركبة. استمر بتمرينك باستخدام الأجهزة ذات المسار الثابت فقط.")
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        with col_log_l:
            ...
            if st.button("بدء 90 ثانية (تضخيم وبناء)", use_container_width=True): 
                pb = st.progress(0)
                for second in range(90): time.sleep(1); pb.progress((second + 1) / 90)
                st.success("انتهى وقت الراحة المعتمد. ارجع للبار فوراً!")
            if st.button("3 دقائق (قوة Power)", use_container_width=True): 
                pb = st.progress(0)
                for second in range(180): time.sleep(1); pb.progress((second + 1) / 180)
        ...
        with col_log_r:
            ...
            final_exercise_name = st.text_input("أدخل اسم التمرين الجديد (باللغة الإنجليزية لتوحيد السجلات في السحابة):") if "يدوي" in selected_exercise or "Custom" in selected_exercise else selected_exercise
            ...
            st.markdown("".join([
                ...
                f"<p><span class='bio-tech'>⚙️ الأداء الميكانيكي (Technique):</span><br><span style='color:#E8ECEF;'>{ex_details.get('technique', 'حافظ على تكنيك سليم ותجنب التأرجح')}</span></p>",
                f"<p><span class='bio-breath'>🫁 التنفس الصحيح (Breathing):</span><br><span style='color:#E8ECEF;'>{ex_details.get('breathing', 'تنفس منتظم مستمر')}</span></p><hr style='border-color:#30363D;'>",
                ...
                f"<p><span class='bio-bad'>❌ الألم السيء والإصابات (Injury Warning):</span><br><span style='color:#E8ECEF;'>{ex_details.get('bad_pain', 'المفاصل أو الأوتار')}</span></p><hr style='border-color:#30363D;'>",
                f"<h5 style='color:#E5B94C; margin:0;'>النطاق العلمي الأمثل: {ex_details.get('reps', '10-12 عدة')}</h5></div>"
            ]), unsafe_allow_html=True)
            ...
            if past_date:
                st.markdown("".join([
                    ...
                    f"<p style='color:#8B949E; margin:0; font-size:14px;'>سجلك التاريخي الأخير ({past_date}):</p>",
                    f"<p style='margin:5px 0 0 0;'><b style='color:#58A6FF; font-size:24px;'>{past_weight} KG</b> <span style='color:#8B949E;'>×</span> <b style='color:#E8ECEF; font-size:20px;'>{past_reps}</b> <span style='color:#8B949E;'>عدات</span></p></div>"
                ]), unsafe_allow_html=True)
            ...
            if st.button("💾 توثيق وتشفير الجولة في السحابة", use_container_width=True):
                ...
                if input_reps == 0: st.success(f"🤖 محرك الذكاء الاصطناعي استنتج أنك حققت {calculated_reps} عدات بناءً على الوزن القديم.")
                ...
                if sync_status: st.success(f"تم تسجيل وتشفير تمرين ({final_exercise_name}) بنجاح تام.")
                else: st.error(sync_message)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with st.form("doms_analysis_form"):
            ...
            doms_location = st.selectbox("أين تركز الألم بشكل رئيسي؟", ["في بطن العضلة المستهدفة (ألم تمدد طبيعي وممتاز)", "في المفاصل أو الأوتار المحيطة (خطر إصابة)", "في أسفل الظهر أو القطنية (تحذير جدي)", "في الرقبة أو الترابيس العلوية (تكنيك خاطئ)"])
            
            if st.form_submit_button("💾 تحليل وحفظ تقرير الاستشفاء", use_container_width=True):
                if "المفاصل" in doms_location or "أسفل الظهر" in doms_location: st.error("⚠️ التقييم يوضح أن التكنيك كان خاطئاً أو أنك استخدمت أوزاناً ثقيلة جداً لدرجة استعانتك بمفاصلك لرفعها. راجع أداءك المرة القادمة فوراً لحماية الأربطة.")
                elif doms_intensity > 8: st.warning("⚠️ الألم المرتفع جداً (أكثر من 8) يعني تمزقاً ليفياً عالياً. يتطلب هذا أخذ راحة سلبية وتجنب تمرين العضلة لـ 72 ساعة القادمة، مع زيادة حصة البروتين.")
                else: st.success("✅ أداء مثالي. ألم العضلة المحتمل يدل على تمزيق الألياف بشكل إيجابي لإعادة بنائها بصلابة وحجم أكبر. استمر في التغذية السليمة والنوم العميق.")
    ...
    with t_clinic:
        ...
        with st.form("clinic_inbody_form"):
            ...
            if st.form_submit_button("💾 تشفير وأرشفة البيانات الطبية", use_container_width=True):
                inbody_payload = {"Date": inb_date.strftime("%Y-%m-%d"), "Weight": inb_weight, "Muscle_Mass": inb_muscle, "Fat_Percentage": inb_fat, "Visceral_Fat": inb_visceral}
                sync_status, sync_msg = secure_push_data("InBody_Logs", inbody_payload)
                if sync_status: st.success("تمت الأرشفة بنجاح. أرقامك الحيوية في أمان تام.")
                else: st.error(sync_msg)
            ...
            if uploaded_meal_img:
                ...
                if st.button("🔍 بدء المسح الضوئي واستخراج الماكروز (Scan Image)", use_container_width=True):
                    with st.spinner("جاري الاتصال بخوادم Vision API المتقدمة... تحليل الأنسجة الكيميائية والأبعاد..."):
                        ...
                        ai_result_html = "".join([
                            "<div class='success-box'><h3 style='margin:0; color:#2EA043;'>🤖 اكتمل التحليل الكيميائي بنجاح!</h3>",
                            ...
                            "<div style='display: flex; justify-content: space-around; text-align: center;'><div><p style='color:#8B949E; margin:0 0 10px 0; font-size:15px;'>البروتين المقدر</p>",
                            f"<h2 style='color:#E8ECEF; margin:0;'>{estimated_protein} g</h2></div><div><p style='color:#8B949E; margin:0 0 10px 0; font-size:15px;'>السعرات المقدرة</p>",
                            f"<h2 style='color:#E8ECEF; margin:0;'>{estimated_calories} kcal</h2></div></div>",
                            "<p style='font-size:13px; color:#8B949E; margin-top:25px; text-align:center;'>تم خصم عملية مسح واحدة من رصيدك. تمت إضافة القيم لعدادك اليومي تلقائياً.</p></div>"
                        ])
    ...
    with t_fuel:
        ...
        with col_fuel_left:
            ...
            fuel_dashboard_html = "".join([
                "<div class='titan-card'><h3 style='margin-top:0;'>📊 لوحة الوقود اللحظية</h3>",
                ...
                f"<p><span class='data-label'>صافي السعرات النهائي بعد المجهود:</span> <br><b style='color:#E8ECEF; font-size:32px;'>{net_calories_consumed} kcal</b></p></div>"
            ])
            ...
            with st.form("health_metrics_save_form"):
                ...
                if st.form_submit_button("💾 توثيق وحفظ يوم التغذية النهائي في السحابة", use_container_width=True):
                    health_payload = {"Date": current_date_str, "Sleep": sleep_hours, "Water": water_liters, "Protein": st.session_state['daily_protein'], "Calories": net_calories_consumed, "Notes": ""}
    ...
    with t_sys:
        ...
        with col_admin_1:
            ...
            if st.button("🔄 بدء فحص وإصلاح قاعدة البيانات", use_container_width=True):
                with st.spinner("جاري المسح العميق والتفاوض مع خوادم Google Workspace..."):
                    ...
                    for rpt in heal_diagnostics: st.markdown(f"<div class='{'success-box' if rpt['status'] == 'success' else 'alert-box'}'>{rpt['msg']}</div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        st.markdown("<hr style='border-color:#30363D; margin:40px 0;'><br>#### 📑 وحدة استخراج تقارير الأداء للعملاء", unsafe_allow_html=True)
